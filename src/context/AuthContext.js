...
//this below is only for the UI development 
'use client';
import { createContext, useContext, useState, useEffect } from 'react';
import { useRouter } from 'next/navigation';
...
export const AuthProvider = ({ children }) => {
  ...
  // 🔹 UI-only login: DUMMY LOGIN, NO API
  const login = async ({ email, password }) => {
    // Create a fake user
    const fakeUser = {
      id: "dev-user-123",
    ...
    // OPTIONAL: Fake token ONLY if your UI checks for it
    document.cookie = "auth-token=dev-token-123; path=/";
